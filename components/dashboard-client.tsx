...
export default function DashboardClient({ mensagens, stats }: DashboardClientProps) {
  const taxaEntrega = stats.totalMensagens > 0 ? ((stats.enviadas / stats.totalMensagens) * 100).toFixed(1) : "0"
  const taxaLeitura = stats.enviadas > 0 ? ((stats.lidas / stats.enviadas) * 100).toFixed(1) : "0"

  const diasSemana = Object.keys(stats.mensagensPorDia)
  const maxMensagens = Math.max(...Object.values(stats.mensagensPorDia), 1)
      ...
      <main className="ml-64 p-6 transition-all duration-300">
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div className="border-4 border-neutral-900 bg-white p-6">
            ...
            <div className="text-4xl font-bold text-neutral-900">{stats.totalContatos}</div>
          ...
          <div className="border-4 border-neutral-900 bg-white p-6">
            ...
            <div className="text-4xl font-bold text-neutral-900">{stats.totalCampanhas}</div>
          ...
          <div className="border-4 border-neutral-900 bg-white p-6">
            ...
            <div className="text-4xl font-bold text-neutral-900">{stats.enviadas}</div>
          ...
          <div className="border-4 border-neutral-900 bg-white p-6">
            ...
            <div className="text-4xl font-bold text-neutral-900">{taxaLeitura}%</div>
            <div className="mt-2 text-xs text-neutral-600">{stats.lidas} mensagens lidas</div>
          </div>
        </div>

        {/* Gráfico de Mensagens por Dia */}
        <div className="mb-6 border-4 border-neutral-900 bg-white p-6">
          <h2 className="mb-6 text-lg font-bold uppercase tracking-wider text-neutral-900">
              ...
              diasSemana.map((dia) => {
                ...
                return (
                  <div key={dia} className="flex flex-1 flex-col items-center">
                    <div className="mb-2 text-xs font-bold text-neutral-900">{count}</div>
                    <div
                      className="w-full border-2 border-neutral-900 bg-neutral-900"
                      style={{ height: `${altura}%`, minHeight: "8px" }}
                    />
                    <div className="mt-2 text-xs text-neutral-600">{dia}</div>
                  </div>
                )
              })
            )}
        ...
        <div className="border-4 border-neutral-900 bg-white">
          <div className="border-b-2 border-neutral-900 p-4">
            <h2 className="text-lg font-bold uppercase tracking-wider text-neutral-900">Mensagens Recentes</h2>
          </div>
